...
function HomeScreen({ wines, onAsk, onMenu, onOpenWine, onCollection, onFavorites, onWishlist, onMustTry, onPairings }){
  ...
  return <div style={{height:'100%',overflowX:'hidden',overflowY:'auto',background:'#fff'}}><div style={{paddingTop:V_STATUS}}/>
    ...
    <main style={{padding:'25px 28px',paddingBottom:V_NAV+68}}>
      ...
      {recent.length>0&&<div style={{display:'flex',gap:13,overflowX:'auto',margin:'14px -28px 0',padding:'0 28px 4px'}}>{recent.map(w=><button key={w.id} onClick={()=>onOpenWine(w.id)} style={{width:105,flexShrink:0,border:0,background:'none',padding:0,textAlign:'left',cursor:'pointer'}}><BottlePhoto wine={w} w={105} h={126} rounded={5}/><div style={{fontFamily:'var(--serif)',fontSize:14,marginTop:8,lineHeight:1.2,color:T.ink}}>{w.name}</div></button>)}</div>}
    </main></div>;
}
