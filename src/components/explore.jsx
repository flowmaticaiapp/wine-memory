// explore.jsx — Explore a Region: learn what grows there, how it tastes, what you own.
// Ported from app/explore.jsx.
...
function ExploreScreen({ region, wines, onBack, onOpenWine, onPick }){
  ...
  return (
    <div style={{ height:'100%', overflow:'auto', background:'#fff' }}>
      ...
      <div style={{ padding:'14px 20px 0', paddingBottom:V_NAV+96 }}>
        {/* region picker */}
        <div style={{ display:'flex', gap:8, overflowX:'auto', margin:'8px -20px 18px', padding:'0 20px' }}>
          {Object.keys(REGIONS).map(name=>{ const on=name===region; return (
            <button key={name} onClick={()=>onPick(name)} style={{ flexShrink:0, padding:'7px 14px', borderRadius:99, cursor:'pointer', background:on?T.ink:'#fff', border:`1px solid ${on?T.ink:T.line2}`, color:on?'#fff':T.ink2, fontFamily:'var(--sans)', fontSize:13.5, fontWeight:on?660:540 }}>{name}</button> ); })}
        </div>

        <div style={{ fontFamily:'var(--mono)', fontSize:11.5, color:T.ink3, letterSpacing:0.3, textTransform:'uppercase' }}>{r.country}</div>
        <h1 style={{ margin:'4px 0 0', fontSize:32, fontWeight:800, letterSpacing:-1 }}>{region}</h1>
        ...
        <div style={{ marginTop:22 }}>
          <Lbl3>Pairs nicely with</Lbl3>
          <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
            {(r.pairs||[]).map(p=> <span key={p} style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'8px 13px', borderRadius:10, background:T.canvas, border:`1px solid ${T.line}`, fontSize:13.5, fontWeight:560, color:T.ink }}><Icon name="fork" size={14} color={accent}/>{p}</span>)}
          </div>
        </div>
        ...
        <div style={{ marginTop:26 }}>
          ...
          </div> : <div style={{ padding:'16px', border:`1px dashed ${T.line2}`, borderRadius:13, background:T.canvas, fontSize:13.5, color:T.ink2, lineHeight:1.5 }}>Nothing from {region} yet. Now you know the grapes to look for — try a {r.grapes[0]} next time you’re shopping.</div>}
        </div>
      </div>
    </div>
  );
